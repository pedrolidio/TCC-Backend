...
class SensorsDataController {
  async show(req, res, next) {
    try {
      const { driving_configuration_id, vehicle_id } = req.query;

      const data = await SensorsDataService.getByDrivingConfigurationIdAndVehicleId(driving_configuration_id, vehicle_id);
